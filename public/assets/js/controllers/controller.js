...
(function() {
    ...
    app.controller('ItemsController', ['$http', function($http) {
            ...
            $http.get('/find').success(function(data) {
                ...
                for (var g = 0; g < item.stuff.gear.length; g++) {
                    ...
                    for (var c = 0; c < co.length; c++) {
                        //if the item name matches the comment's itemlink add the comment to the itemObj.commentData inside gear array
        ...
        //turn on store vis
        var showStore = function() { self.store.state = true };
        //turn off login vis
        var hideLogin = function() { self.keep.state = false }
        //turn on current user vis
        var currentUser = function() { self.current.state = true }        
